...
상위·하위 쿼리를 하나의 동작으로 묶어서 처리하지 못하는 경우(평탄화 불가)를 말합니다. 서브쿼리 안에 A 의 구문이 있거나 LIMIT / window 함수가 있을 때 발생하며, 평탄화 못 한 서브쿼리는 별도 단계로 평가되어 결과가 temp 에 저장됩니다 (**Derived table materialize**).

B 는 사실 A 가 한 단계 더 깊이 발생한 메타 케이스입니다 — 서브쿼리 안의 A 가 평탄화를 막아 별도의 temp 단계를 만듭니다.

같은 원리로, **하위 depth 의 temp 는 (사용자 정의) 인덱스를 가질 수 없습니다**.  
상위 depth 가 그 temp 를 참조하면 풀스캔이 강제되고, 위에 A 의 구문이 더 걸리면 temp 가 한 층 더 쌓입니다.
...
깊어진 서브쿼리의 안쪽 블록이 temp (materialized ) 되면, 그 temp 는 원본 테이블의 인덱스를 물려받지 않습니다. 상단의 연산에서 그 temp 를 참조해야 한다면, temp 위에 만들어지는 연산은 인덱스 조건을 쓴다 하여도 풀스캔이 강제됩니다. 이 풀스캔이 또 외곽 temp 에 얹히고 그 위에 또 얹히면, 층마다의 비용이 서로 곱해집니다.

3.2 의 "id 선행 확정" 으로도 이런 중첩 temp 를 깨끗하게 자를 수 없는 구조가 있습니다. 가령 아래와 같은 모양에서 상위의 모양에 temp 의 결과값을 사용한다고 가정해봅시다.

e.g.) 특정 칼럼의 sum 조건을 이어받아 상단의 연산에 사용해야 하는 경우가 있다고 합시다. 이 경우 3.1 처럼 id 로 횡으로 깔끔히 잘리지 않습니다. 
...
단 Key 를 기준으로 횡으로 자르는 것과 sub query 영역을 어플리케이션 영역에 보관하는 종으로 자르는 것의 차이는, 여기선 죽어있던 인덱스가 활용될 여지가 있다는 점입니다.
