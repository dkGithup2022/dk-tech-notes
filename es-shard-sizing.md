...
- 데이터 타입 보정 / noindex 처리 등 다른 튜닝 요소: 2~5% 향상
- 샤드 사이즈 조절을 통한 물리적 병목 제거: indexing/query 양쪽 2~3배 향상

**본 글은 후자에 초점을 둔다.** indexing 쪽은 같은 클러스터의 색인 부하 측정으로 별도 확인했고, 본 글은 검색 측에 집중한다.

### 2.1 병목 진단

데이터 노드 CPU 90% 이상, 다른 자원 20% 미만 — 병목이 원인인 데이터 노드임을 확인.

ES 7 에서 한 샤드의 검색은 단일 쓰레드로 처리됨 ([→ 3.1 샤드 사이즈에 따른 퍼포먼스 하락](#31-샤드-사이즈에-따른-퍼포먼스-하락)). 샤드 1개의 처리량 한계를 직접 측정하는 쪽으로 진행.
...
- 부하 패턴 — 꾸준 부하 위에 QPS를 계단식으로 올리며, 계단식으로 성능이 하락하는 지점을 파악한다. (문서 갯수, 데이터 크기)
- 관찰 곡선 (재현값):
...
본 작업에서도 같은 단계적 성능하락이 확인됐고, primary shard 크기를 늘려 퍼포먼스를 확보했다.

도식과 차이는, 실제 작업에서는 아래 세미나보다 상승폭이 더 컸다.
700 QPS 수준이던 것을 샤드 크기 조정 이후에 2,000 QPS 이상 확보할 수 있었다.
