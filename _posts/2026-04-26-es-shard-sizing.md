...
탑다운 1,000 QPS 목표를 받은 상품 검색용 신규 ES 클러스터에서, single-shard test로 노드 단위 처리량 한계를 측정하고 그 임계 미만으로 샤드를 사이징해 단일 노드 처리량을 **4배 이상**(500 → 2,000+ QPS, P95 latency SLO 기준 안정 구간)으로 확장. 클러스터 전체 1,000 QPS 목표는 단일 노드 2,000+ × N 노드의 여유로 충당했고, 본 글은 그 단위 수치를 만든 측정 작업에 한정한다.
...
파이프라인 어디서 막히는지부터 봤다. 데이터 노드 CPU 90% 이상, 다른 자원 20% 미만 — CPU만 단독으로 튀는 패턴이라 한 샤드 단위 핫스팟을 시사한다.
